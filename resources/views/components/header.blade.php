...
<header class="fixed top-0 left-0 right-0 z-50 h-16 bg-white border-b border-slate-200 shadow-lg">
    <div class="flex justify-between items-center h-full px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center space-x-2 sm:space-x-6">
            ...
            @isset($user)
                <!-- Desktop: User Info with photo or default icon -->
                <div class="hidden sm:flex items-center space-x-3 px-3 py-1 rounded-lg bg-white border border-slate-200 shadow-sm cursor-pointer"
                     onclick="document.getElementById('profileModal').classList.remove('hidden')">
                    <div class="w-6 h-6 sm:w-8 sm:h-8 rounded-full overflow-hidden shadow-lg">
                        @if($user->photo)
                ...
                <!-- Mobile: initials or default icon -->
                <div class="sm:hidden flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 text-white font-semibold shadow-lg cursor-pointer"
                     onclick="document.getElementById('profileModal').classList.remove('hidden')">
                    @if($user->photo)
                        <img src="{{ asset('storage/'.$user->photo) }}" alt="Profile Photo" class="w-full h-full object-cover rounded-full">
                    @else
                        {{ strtoupper(substr($user->name ?? 'G', 0, 1)) }}
                    @endif
                </div>
