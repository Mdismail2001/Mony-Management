...
@section('content')
<div class="h-screen overflow-hidden bg-slate-950 flex flex-col">
    {{-- Header --}}
            ...
            <section class="grid {{ $gridClass }} gap-6 mb-8">
                @foreach($cards as $card)
                    ...
                    <a href="{{ $card['route'] ?? '#' }}" class="group relative bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl p-6 hover:border-slate-700 transition-all hover:shadow-xl hover:shadow-slate-900/50">
                        ...
                        <!-- Content -->
                        <h3 class="text-xl font-medium text-white mb-2">{{ $card['title'] }}</h3>
                        <div class="space-y-1">
                            <p class="text-sm font-bold text-slate-400">{{ $card['value_1'] }}</p>
                            @if(!empty($card['value_2']))
                                <p class="text-sm font-semibold text-slate-400">{{ $card['value_2'] }}</p>
                            @endif
                        </div>
                    </a>
                @endforeach
